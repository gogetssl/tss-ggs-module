var component = {
    ...
    methods: {
        ...
        checkClearMainSearchButton: function (event) {
            const clearButton = $(event.target).parent().find(".lu-clear-main-search-button");

            (clearButton.length > 0 && typeof event.target.value != "undefined" && event.target.value.length > 0) ?
                this.enableButton(clearButton) : this.disableButton(clearButton);
        },

        clearMainSearch: function (event) {
            const clearButton = $(event.target).parent().parent().find(".lu-clear-main-search-button");
            const inputSearchField = $(event.target).parent().parent().find(".lu-table-search");
            ...
            this.disableButton(clearButton);
            this.searchDataEnter(event);
        },

        toggleFilters: function () {
            if (this.filtersVisible_)
            {
                this.hideFilters();
                this.loadAjaxData_();
            }
        ...
        showFilters: function () {
            $("#" + this.cid + " .lu-filter-input-container").slideDown("fast");

            this.$nextTick(function () {
                $("#" + this.cid + " .lu-filter-input").first().focus();
            }.bind(this));

            this.filtersVisible_ = true;
        },
        hideFilters: function () {
            $("#" + this.cid + " .lu-filter-input-container").slideUp("fast");

            $("#" + this.cid + " .lu-filter-input").each((index, input) => {
                input.value = "";
            });

            this.filtersVisible_ = false;
            this.ajaxData_['filters'] = [];
        },
        ...
        filterRecords: function () {
            ...
            $("#" + this.cid + " .lu-filter-input").each((index, input) => {
                filtersValues[input.name] = input.value;
            });

            this.ajaxData_['filters'] = filtersValues;

            this.loadAjaxData_();
        },

        checkClearFilterButton: function (event) {
            const clearButton = $(event.target).parent().find(".lu-filter-input-clear-button");

            (clearButton.length > 0 && typeof event.target.value != "undefined" && event.target.value.length > 0) ?
                this.enableButton(clearButton) : this.disableButton(clearButton);
        },

        enableButton: function (clearButton) {
            clearButton.removeAttr('disabled')
        },

        disableButton: function (clearButton) {
        ...
        clearFilterInput: function (event) {
            ...
            const inputField = $(event.target).parent().parent().find(".lu-filter-input");

            if (inputField.length > 0)
            {
                inputField.val('');

                const clearButton = $(event.target).parent().parent().find(".lu-filter-input-clear-button");
                this.disableButton(clearButton)
        ...
        buildDraggable: function () {
            ...
            const table = $("#" + this.cid_ + " tbody");

            new Sortable(table[0], {
                animation: 150,
                ghostClass: 'lu-bg-grey',
        ...
        onDropRow: function (value) {
            if (!this.reorderProviderAction_)
            {
                return;
            }

            var droppedRow            = $(value.item);
            var droppedRowId= droppedRow.length > 0 ? droppedRow[0].getAttribute("data-order-id") : null;
            var recordsOrdered  = [];

            $("#" + this.cid_ + " tbody tr").each((order, item) => {
                recordsOrdered[order] = item.getAttribute("data-order-id");
            });
